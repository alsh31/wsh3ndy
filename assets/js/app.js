...
function findConflicts(course){
  if(course.isExam) return [];
  return courses.filter(c=>{
    if(c.isExam) return false;
...
function renderCards(){
  ...
  courses.filter(c=>!c.isExam).forEach(course=>{
...
function renderList(){
  ...
  const lectures = courses.filter(c=>!c.isExam);
  if(lectures.length===0){
    listContainer.innerHTML = '<div class="empty-msg">لا توجد مواد بعد — اضغط "إضافة مادة" للبدء</div>';
    return;
  }
  const sorted = [...lectures].sort((a,b)=> DAYS.indexOf(a.day)-DAYS.indexOf(b.day) || toMinutes(a.start)-toMinutes(b.start));
  sorted.forEach(c=>{
...
function renderExams(){
  ...
  const exams = courses.filter(c=>c.isExam);
  ...
  const sorted = [...exams].sort((a,b)=> (a.dateStart||'').localeCompare(b.dateStart||''));
  sorted.forEach(c=>{
    const row=document.createElement('div');
    row.className='exam-row';
    row.innerHTML = `
      <div class="exam-date">
        <b>${formatExamDate(c.dateStart)||'—'}</b>
        <span>${c.day}</span>
      </div>
      <div class="info">
        <b>${c.emoji? c.emoji+' ':''}${escapeHtml(c.name)}</b>
        <small>${formatTimeDisplay(c.start)} - ${formatTimeDisplay(c.end)}${c.room? ' • '+escapeHtml(c.room):''}${c.instructor? ' • '+escapeHtml(c.instructor):''}</small>
      </div>
      <div class="row-actions">
        <button class="icon-btn" data-edit="${c.id}" title="تعديل">✎</button>
        <button class="icon-btn danger" data-del="${c.id}" title="حذف">✕</button>
  ...
  examsContainer.querySelectorAll('[data-edit]').forEach(b=>
    b.addEventListener('click', ()=> openModal(b.dataset.edit)));
  examsContainer.querySelectorAll('[data-del]').forEach(b=>
    b.addEventListener('click', ()=> deleteCourse(b.dataset.del)));
}

function updateTotalHours(){
  const total = courses.filter(c=>!c.isExam).reduce((sum,c)=> sum + (toMinutes(c.end)-toMinutes(c.start)), 0);
...
function openModal(id){
  ...
  if(id){
    ...
    document.getElementById('f_isExam').checked = !!c.isExam;
    ...
    document.getElementById('f_isExam').checked = false;
...
document.getElementById('saveBtn').addEventListener('click', async ()=>{
  ...
  const isExam = document.getElementById('f_isExam').checked;
  ...
  const courseData = {
    id: editingId || uid(),
    name, day, room, start, end, dateStart, dateEnd, instructor, color: selectedColor, emoji: selectedEmoji, isExam
  };
